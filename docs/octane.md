...
`--max-requests` (дефолт 500; задаётся внешним пакетом [`laravel/octane`](https://laravel.com/docs/octane),
а не этим пакетом) периодически перезапускает воркер как страховка от утечек памяти —
...
## Admin UI и `octane:reload`

MoonShine admin UI (реализован, optional — см. [docs/moonshine.md](moonshine.md)) меняет
`enabled` модулей по HTTP **внутри** воркера через `EnableModuleUseCase` /
`DisableModuleUseCase` / `RemoveModuleUseCase`. Поскольку loader pipeline прогоняется
один раз на старте воркера и повторно не запускается, после такой мутации нужен
`octane:reload` (или эквивалентный перезапуск воркеров) — иначе включённый модуль не
догрузится, а отключённый продолжит работать до следующего reload. Это та же
операционка, что и для CLI-команд enable/disable/remove.

Правка feature-значений (`settings.values`) из формы UI под это правило **не**
подпадает: значения читаются из `state.json` на каждый запрос и видны со следующего
запроса без reload (см. выше).
