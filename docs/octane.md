...
`--max-requests` (по умолчанию 500) периодически перезапускает воркер как страховка
от утечек памяти — это не замена `octane:reload` и на контракт выше не влияет.
