...
- Запускай один упорядоченный pipeline по `ModuleRegistry::loadOrder()` и лоадерам, отсортированным по `priority()`, пропуская отключённые модули.
- При добавлении лоадеров сохраняй смысл приоритетов: config и providers первыми, ресурсы до routes, commands после routes, MoonShine последним.
