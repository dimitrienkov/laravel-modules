...
- Оставляй `LoaderInterface` тонким: только `load(Module): LoadReport` и `priority(): int`; не добавляй ключи, имена или whitelist-контракты манифеста.
- Каждый лоадер должен быть идемпотентным; если ожидаемых файлов или директорий модуля нет — делай ранний `return LoadReport::skipped(<reason>)`, а на успешной загрузке возвращай `LoadReport::applied(<artifacts>)`.
